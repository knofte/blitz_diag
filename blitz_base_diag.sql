...
BEGIN
  ...
  (cur_section, 'InnoDB Buffer Pool', 'Buffer pool efficiency is key to read performance — see #02 for tuning strategies:',
   CONCAT('Size: ', @@innodb_buffer_pool_size,
          ', Reads: ', (SELECT VARIABLE_VALUE FROM information_schema.GLOBAL_STATUS WHERE VARIABLE_NAME = 'Innodb_buffer_pool_reads'),
          ', Read Requests: ', (SELECT VARIABLE_VALUE FROM information_schema.GLOBAL_STATUS WHERE VARIABLE_NAME = 'Innodb_buffer_pool_read_requests')));

  -- Temp Table Usage
  SET cur_section := cur_section + 1;
  INSERT INTO blitz_output VALUES
  (cur_section, 'Temp Tables', 'Disk-based temp tables are expensive — check ratio of disk to memory-created temp tables (#03):',
   CONCAT('Disk: ', (SELECT VARIABLE_VALUE FROM information_schema.GLOBAL_STATUS WHERE VARIABLE_NAME = 'Created_tmp_disk_tables'),
          ', All Temp: ', (SELECT VARIABLE_VALUE FROM information_schema.GLOBAL_STATUS WHERE VARIABLE_NAME = 'Created_tmp_tables'),
  ...
  SELECT cur_section, 'Foreign Key Mismatches', 'These foreign keys have mismatched column types (#06):',
         GROUP_CONCAT(CONCAT(fk_schema, '.', fk_table, '(', fk_column, ') -> ', ref_table, '.', ref_column, ' [', fk_column_type, ' ≠ ', ref_column_type, ']') SEPARATOR ' | ')
  FROM (
